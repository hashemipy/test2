...
class KK_Shortcodes {
    ...
    private function __construct() {
        ...
        add_shortcode('k_categories_panel_data', [$this, 'categories_panel_data']);
    }
    ...
    public function stories_data() {
        ...
        foreach ($stories as $story) {
            $transformed[] = [
                ...
                'button_text' => $story['button_text'] ?? 'مشاهده کالکشن'
            ];
        }
    ...
    public function sales_data() {
        ...
        if ($end_timestamp === 0 || time() >= $end_timestamp) {
            ...
            // استفاده از wc_get_product_ids_on_sale که مستقیم از دیتابیس ID ها رو میگیره
            $on_sale_ids = wc_get_product_ids_on_sale();
            
            // فقط محصولات موجود رو فیلتر می‌کنیم با یک query بهینه
            if (!empty($on_sale_ids)) {
                $sale_ids = wc_get_products([
                    ...
                    'limit' => -1,
                    'return' => 'ids'  // فقط ID برگردون، نه کل object
                ]);
            } else {
                $sale_ids = [];
            }
    ...
    public function categories_data() {
        ...
        foreach ($categories as $category) {
            ...
            // Check if this category has a WooCommerce category ID set
            if (!empty($category['wc_category_id'])) {
                $processed_category['wc_category_id'] = $category['wc_category_id'];
            }
            $processed_categories[] = $processed_category;
        }
        return json_encode(['categories' => $processed_categories]);
    }
    ...
    public function accordion_products_data() {
        ...
        $data['latest_view_all_text'] = get_option('k_latest_view_all_text', 'نمایش همه ←');
        $data['latest_gradient_start'] = get_option('k_latest_gradient_start', '#f093fb');
        $data['latest_gradient_end'] = get_option('k_latest_gradient_end', '#f5576c');
        $data['latest_button_bg'] = get_option('k_latest_button_bg', 'rgba(255, 255, 255, 0.3)');
        $data['latest_button_text'] = get_option('k_latest_button_text', '#ffffff');
        $data['latest_icon'] = get_option('k_latest_icon', '✨');
        $data['latest_title'] = get_option('k_latest_title', 'همه محصولات');
        $data['latest_layout'] = get_option('k_latest_layout', 'scroll');
        $data['latest_default_open'] = get_option('k_latest_default_open', '1');
        $data['latest_scroll_limit'] = get_option('k_latest_scroll_limit', 20);
        $data['latest_grid_limit'] = get_option('k_latest_grid_limit', 10);
        $data['latest_order'] = get_option('k_latest_order', 0);
        
        $latest_category_ids = KK_Helper::ensure_array(get_option('k_latest_category', []));
        
        if (!empty($latest_category_ids)) {
            $latest_products = [];
            foreach ($latest_category_ids as $cat_id) {
                $latest_products = array_merge($latest_products, KK_Helper::get_products_by_category($cat_id));
            }
            $data['latest_products'] = KK_Helper::filter_in_stock(array_unique($latest_products));
        } else {
            // اگر دسته‌بندی انتخاب نشده باشد، تمام محصولات نمایش داده شوند
            $data['latest_products'] = wc_get_products([
                'limit' => -1,
                'orderby' => 'date',
                'order' => 'DESC',
                'status' => 'publish',
        ...
        foreach ($sections as $key) {
            ...
            $data["{$key}_view_all_text"] = get_option("k_{$key}_view_all_text", 'نمایش همه ←');
            ...
            $data["{$key}_order"] = get_option("k_{$key}_order", 0);
    ...
    public function bottom_bar_data() {
        return json_encode([
            ...
            'show_on_cart' => get_option('k_bottom_bar_show_cart', '1') === '1',
            'show_on_sendcode' => get_option('k_bottom_bar_show_sendcode', '1') === '1',
            'show_on_sales' => get_option('k_bottom_bar_show_sales', '1') === '1',
    ...
    public function categories_panel_data() {
        $enabled = get_option('k_categories_panel_enabled', '0') === '1';
        $selected_cat_ids = get_option('k_categories_panel_categories', []);
        
        $categories = [];
        if ($enabled && !empty($selected_cat_ids)) {
        ...
        return json_encode([
            'enabled' => $enabled,
            'auto_open' => get_option('k_categories_panel_auto_open', '0') === '1',
            'auto_open_category' => (int)get_option('k_categories_panel_auto_open_category', 0),
            'button_label' => get_option('k_categories_panel_button_label', 'دسته بندی'),
            'bg_color' => get_option('k_categories_panel_bg_color', '#ffffff'),
            'icon' => get_option('k_categories_panel_icon', ''),
