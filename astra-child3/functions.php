...
/**
 * Image Performance Optimizations for Iran Servers
 * - Add lazy loading to all images
 * - Add fetchpriority for above-the-fold images
 * - Add decoding="async" for better performance
 */
function khoshtip_optimize_images($attr, $attachment, $size) {
...
add_filter('wp_get_attachment_image_attributes', 'khoshtip_optimize_images', 10, 3);

/**
 * Add lazy loading to post thumbnails
 */
function khoshtip_lazy_load_post_thumbnail_html($html, $post_id, $post_thumbnail_id, $size, $attr) {
    if (strpos($html, 'loading=') === false) {
        $html = str_replace('<img', '<img loading="lazy" decoding="async"', $html);
    }
    return $html;
}
add_filter('post_thumbnail_html', 'khoshtip_lazy_load_post_thumbnail_html', 10, 5);

/**
 * Add lazy loading to content images
 */
function khoshtip_lazy_load_content_images($content) {
    if (is_admin()) {
        return $content;
    }
...
add_filter('the_content', 'khoshtip_lazy_load_content_images', 20);

/**
 * Preload critical resources for faster page load
 */
function khoshtip_preload_critical_resources() {
    $font_source = get_option('khoshtip_font_source', 'local');
    
    // Preload local font if using local fonts
    if ($font_source !== 'google') {
        echo '<link rel="preload" href="' . get_stylesheet_directory_uri() . '/assets/fonts/Vazirmatn-Regular.woff2" as="font" type="font/woff2" crossorigin>' . "\n";
    }
    
    // DNS prefetch for any external resources that might still be needed
    echo '<link rel="dns-prefetch" href="//fonts.googleapis.com">' . "\n";
}
add_action('wp_head', 'khoshtip_preload_critical_resources', 1);
