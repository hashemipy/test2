...
<div id="product-<?php the_ID(); ?>" <?php wc_product_class( 'khoshtip-single-product', $product ); ?>>
    ...
    <div class="khoshtip-product-container">
        ...
        <div class="khoshtip-product-gallery">
            ...
            <?php
            $video_data = function_exists('astra_get_product_video') ? astra_get_product_video($product->get_id()) : false;
            $has_video = $video_data && !empty($video_data['url']);
            ?>
            ...
            if ( $attachment_ids || $has_video ) : ?>
                <div class="khoshtip-gallery-thumbnails">
                    ...
                    <!-- Added video thumbnail in gallery -->
                    <?php if ( $has_video && !empty($video_data['thumbnail']) ) : ?>
                        <div class="khoshtip-thumbnail product-video-thumb video-thumb-trigger"
                             data-image="<?php echo esc_url( $video_data['thumbnail'] ); ?>"
                             data-video-url="<?php echo esc_url( $video_data['url'] ); ?>"
                             ...
                             data-product-image="<?php echo esc_url( $video_data['thumbnail'] ); ?>">
                            <img src="<?php echo esc_url( $video_data['thumbnail'] ); ?>" alt="<?php echo esc_attr( $product->get_name() ); ?> - ویدیو">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    thumbnails.forEach(thumb => {
        thumb.addEventListener('click', function() {
            ...
            // Don't change main image for video thumbnails
            if (!this.classList.contains('product-video-thumb')) {
    ...
    if (typeof khoshtipVariationsData !== 'undefined') {
        ...
        if (variationForm) {
            ...
            jQuery(variationForm).on('found_variation', function(event, variation) {
                if (variation.image && variation.image.src) {
                    ...
                    const matchingThumb = Array.from(thumbnails).find(thumb => 
                        !thumb.classList.contains('product-video-thumb') && (
            ...
            jQuery(variationForm).on('reset_image', function() {
                const defaultImage = Array.from(thumbnails).find(t => !t.classList.contains('product-video-thumb'));
                if (defaultImage) {
