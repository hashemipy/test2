...
class Khoshtip_Google_Auth {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_check_phone_required', array($this, 'check_phone_required'));
        add_action('wp_ajax_nopriv_check_phone_required', array($this, 'check_phone_required'));
        
        add_action('wp_ajax_save_user_phone', array($this, 'save_user_phone'));
        add_action('wp_ajax_nopriv_save_user_phone', array($this, 'save_user_phone'));
    }
    ...
    private function login_or_register_user($user_info) {
        ...
        update_user_meta($user_id, 'phone_required', '1');
        
        return get_user_by('id', $user_id);
    }
    
    // مدیریت AJAX request برای بررسی نیاز به شماره موبایل
    public function check_phone_required() {
        check_ajax_referer('google_auth_nonce', 'nonce');
        ...
        $phone_required = get_user_meta($user_id, 'phone_required', true);
        $billing_phone = get_user_meta($user_id, 'billing_phone', true);
        
        // اگر علامت phone_required دارد و شماره موبایل ندارد
        if ($phone_required === '1' && empty($billing_phone)) {
            wp_send_json_success(array('required' => true));
        } else {
            wp_send_json_success(array('required' => false));
        }
    }
    
    // مدیریت AJAX request برای ذخیره شماره موبایل
    public function save_user_phone() {
        check_ajax_referer('google_auth_nonce', 'nonce');
        ...
        // ذخیره شماره در فیلد billing_phone
        update_user_meta($user_id, 'billing_phone', $phone);
        
        // حذف علامت phone_required
        delete_user_meta($user_id, 'phone_required');
        
        wp_send_json_success(array('message' => 'شماره موبایل با موفقیت ثبت شد'));
    }
}
